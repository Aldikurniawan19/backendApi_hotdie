    ...
    <div class="max-w-xl">
        ...
        <form method="POST" action="{{ route('admin.coupons.store') }}">
            ...
            <div class="bg-surface-card border border-surface-border rounded-xl p-6 space-y-5">
                <h3 class="text-gray-900 font-semibold text-lg">Kupon Baru</h3>
                <div>
                    <label class="block text-sm font-medium text-gray-500 mb-2">Kode Kupon *</label>
                    <input type="text" name="code" value="{{ old('code') }}" required class="w-full px-4 py-2.5 bg-surface border border-surface-border rounded-lg text-gray-900 placeholder-gray-400 focus:outline-none focus:border-brand text-sm font-mono uppercase" placeholder="PROMO20">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="discount_value" value="{{ old('discount_value') }}" required min="0" step="0.01" class="w-full px-4 py-2.5 bg-surface border border-surface-border rounded-lg text-gray-900 placeholder-gray-400 focus:outline-none focus:border-brand text-sm" placeholder="20">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="min_purchase" value="{{ old('min_purchase', 0) }}" min="0" class="w-full px-4 py-2.5 bg-surface border border-surface-border rounded-lg text-gray-900 placeholder-gray-400 focus:outline-none focus:border-brand text-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-500 mb-2">Berlaku Sampai</label>
                        <input type="date" name="expires_at" value="{{ old('expires_at') }}" class="w-full px-4 py-2.5 bg-surface border border-surface-border rounded-lg text-gray-900 focus:outline-none focus:border-brand text-sm">
                    </div>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-500 mb-2">Deskripsi</label>
                    <input type="text" name="description" value="{{ old('description') }}" class="w-full px-4 py-2.5 bg-surface border border-surface-border rounded-lg text-gray-900 placeholder-gray-400 focus:outline-none focus:border-brand text-sm" placeholder="Diskon spesial untuk pelanggan baru">
                </div>
