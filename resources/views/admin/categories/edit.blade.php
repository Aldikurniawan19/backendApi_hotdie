        ...
        <form method="POST" action="{{ route('admin.categories.update', $category) }}" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                ...
                <div class="lg:col-span-2 bg-surface-card border border-surface-border rounded-xl p-6 space-y-5">
                    <h3 class="text-gray-900 font-semibold text-lg">Edit Kategori: {{ $category->name }}</h3>
                    <div>
                        <label class="block text-sm font-medium text-gray-500 mb-2">Nama Kategori *</label>
                        <input type="text" name="name" id="input-name" value="{{ old('name', $category->name) }}" required class="w-full px-4 py-2.5 bg-surface border border-surface-border rounded-lg text-gray-900 placeholder-gray-400 focus:outline-none focus:border-brand text-sm" placeholder="Contoh: T-Shirt">
                        @error('name') <p class="text-red-400 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-500 mb-2">Warna Background</label>
                        <div class="flex items-center gap-3">
                            <input type="color" name="background_color" id="input-bgcolor" value="{{ old('background_color', $category->background_color) }}" class="w-10 h-10 rounded-lg border border-surface-border cursor-pointer bg-transparent">
                    ...
                    <div>
                        ...
                        <div id="upload-area" class="border-2 border-dashed border-surface-border rounded-lg p-6 text-center hover:border-brand/50 transition cursor-pointer" onclick="document.getElementById('image-input').click()">
                            <div id="preview-container" class="{{ $category->image_url ? '' : 'hidden' }} mb-3">
                                <img id="preview-image" src="{{ $category->image_url }}" class="w-20 h-20 object-cover rounded-lg mx-auto" alt="">
                            </div>
                    ...
                    <div class="flex items-center gap-3">
                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $category->is_active) ? 'checked' : '' }} class="w-4 h-4 rounded border-surface-border bg-surface text-brand focus:ring-brand">
                    ...
                    <div class="bg-white border border-gray-200/80 rounded-2xl p-6 shadow-sm">
                        ...
                        <div id="preview-card-bg" class="h-[140px] rounded-xl overflow-hidden shadow-sm relative flex bg-[#F5E6CC] max-w-sm mx-auto transition-colors duration-200">
                            ...
                            <div class="w-[120px] h-full bg-gray-200 relative overflow-hidden flex items-center justify-center flex-shrink-0">
                                <img id="preview-card-image" src="{{ $category->image_url }}" class="{{ $category->image_url ? '' : 'hidden' }} w-full h-full object-cover">
                                <div id="preview-card-placeholder-icon" class="{{ $category->image_url ? 'hidden' : '' }} absolute inset-0 flex items-center justify-center bg-gray-300/30">
