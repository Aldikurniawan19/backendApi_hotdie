        ...
        <form method="POST" action="{{ route('admin.banners.update', $banner) }}" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                ...
                <div class="lg:col-span-2 bg-surface-card border border-surface-border rounded-xl p-6 space-y-5">
                    <h3 class="text-gray-900 font-semibold text-lg">Edit Banner: {{ $banner->offer_text }}</h3>
                    <div>
                        <label class="block text-sm font-medium text-gray-500 mb-2">Tag *</label>
                        <input type="text" name="tag" id="input-tag" value="{{ old('tag', $banner->tag) }}" required class="w-full px-4 py-2.5 bg-surface border border-surface-border rounded-lg text-gray-900 focus:outline-none focus:border-brand text-sm" placeholder="#WINTER SALE">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-500 mb-2">Teks Promo *</label>
                        <input type="text" name="offer_text" id="input-offer" value="{{ old('offer_text', $banner->offer_text) }}" required class="w-full px-4 py-2.5 bg-surface border border-surface-border rounded-lg text-gray-900 focus:outline-none focus:border-brand text-sm" placeholder="35% Off">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-500 mb-2">Deskripsi</label>
                        <input type="text" name="description" id="input-desc" value="{{ old('description', $banner->description) }}" class="w-full px-4 py-2.5 bg-surface border border-surface-border rounded-lg text-gray-900 focus:outline-none focus:border-brand text-sm" placeholder="Discover our latest products">
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-500 mb-2">Warna Background</label>
                            <div class="flex items-center gap-3">
                                <input type="color" name="background_color" id="input-bgcolor" value="{{ old('background_color', $banner->background_color) }}" class="w-10 h-10 rounded-lg border border-surface-border cursor-pointer bg-transparent">
                    ...
                    <div>
                        ...
                        <div id="upload-area" class="border-2 border-dashed border-surface-border rounded-lg p-6 text-center hover:border-brand/50 transition cursor-pointer" onclick="document.getElementById('image-input').click()">
                            <div id="preview-container" class="{{ $banner->image_url ? '' : 'hidden' }} mb-3">
                                <img id="preview-image" src="{{ $banner->image_url }}" class="w-32 h-16 object-cover rounded-lg mx-auto" alt="">
                            </div>
                            <div id="upload-placeholder">
                                @if($banner->image_url)
                                    <p class="text-gray-500 text-sm">Klik untuk ganti gambar</p>
                                @else
                    ...
                    <div class="flex items-center gap-3">
                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $banner->is_active) ? 'checked' : '' }} class="w-4 h-4 rounded border-surface-border bg-surface text-brand focus:ring-brand">
                        ...
                        <div id="preview-banner-bg" class="h-[200px] w-full rounded-2xl overflow-hidden shadow-sm relative flex bg-[#F5E6CC] max-w-sm mx-auto transition-colors duration-200">
                            ...
                            <div class="w-[40%] h-full p-4 flex-shrink-0 flex items-center justify-center">
                                <div class="w-full h-full rounded-lg bg-gray-200/80 overflow-hidden relative flex items-center justify-center shadow-sm">
                                    <img id="preview-banner-image" src="{{ $banner->image_url }}" class="{{ $banner->image_url ? '' : 'hidden' }} w-full h-full object-cover">
                                    <div id="preview-banner-placeholder-icon" class="{{ $banner->image_url ? 'hidden' : '' }} absolute inset-0 flex items-center justify-center bg-gray-300/30">
