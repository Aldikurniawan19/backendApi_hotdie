            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                ...
                <div class="lg:col-span-2 bg-white border border-gray-200/80 rounded-2xl p-7 space-y-6 shadow-sm">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="input-name" value="{{ old('name', $product->name) }}" required
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="price" id="input-price" value="{{ old('price', $product->price) }}" required step="0.01" min="0"
                                   class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-gray-800 placeholder-gray-400 focus:outline-none focus:border-brand focus:ring-2 focus:ring-brand/10 focus:bg-white text-sm transition-all duration-200">
                    ...
                    <div>
                        ...
                        <input type="text" name="category" id="input-category" value="{{ old('category', $product->category) }}" required
                               class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-gray-800 placeholder-gray-400 focus:outline-none focus:border-brand focus:ring-2 focus:ring-brand/10 focus:bg-white text-sm transition-all duration-200">
                    ...
                    <div class="space-y-3 pt-1">
                        ...
                        <div class="flex items-center gap-3">
                            <input type="checkbox" name="is_trending" id="is_trending" value="1" {{ $product->is_trending ? 'checked' : '' }}
                                   class="w-4 h-4 rounded border-gray-300 bg-gray-50 text-brand focus:ring-brand focus:ring-offset-0">
                    ...
                    <div class="bg-white border border-gray-200/80 rounded-2xl p-6 shadow-sm">
                        ...
                        <div class="border border-gray-100 rounded-2xl overflow-hidden bg-gray-50/50 shadow-sm max-w-sm mx-auto">
                            ...
                            <div class="aspect-square w-full bg-gray-200 relative overflow-hidden flex items-center justify-center">
                                <img id="preview-card-image" src="{{ $product->image_url }}" data-existing-url="{{ $product->image_url }}" class="{{ $product->image_url ? '' : 'hidden' }} w-full h-full object-cover">
                                <div id="preview-card-placeholder" class="absolute inset-0 bg-gradient-to-br from-[#E8F5E9] to-[#B2DFDB] flex items-center justify-center {{ $product->image_url ? 'hidden' : '' }}">
                                    <span id="preview-card-initial" class="text-[#1E7A5C] text-5xl font-bold opacity-30">{{ $product->name ? strtoupper(substr($product->name, 0, 1)) : '?' }}</span>
                                </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            imageInput.addEventListener('change', function() {
                if (this.files && this.files[0]) {
                    ...
                    const existingUrl = cardImage.getAttribute('data-existing-url');
                    if (existingUrl) {
                        cardImage.src = existingUrl;
                        cardImage.classList.remove('hidden');
                        cardPlaceholder.classList.add('hidden');
                    } else {
